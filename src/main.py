...
from src.models.image_model import (
    ImageUploadRequest, UploadResponse, ImageResponse,
    ImageListResponse, DeleteResponse, ErrorResponse
)
...
async def health_check():
    """Health check endpoint"""
    return {"status": "healthy", "service": "instagram-image-service", "version": "1.0.0"}
...
async def get_image(
    ...
    include_url: bool = Query(default=True, description="Include download URL in response")
):
    """Get image metadata and download URL"""
    try:
        result = image_service.get_image(
            image_id=image_id,
            include_download_url=include_url
        )
